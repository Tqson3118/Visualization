...
2. Research chuẩn UML use case (1 trang ghi chú docs/work/diagram-notes.md): quy tắc «include» (nét liền, tam giác rỗng, hướng UC gọi → UC bị include) + «extend» (nét đứt, tam giác rỗng, hướng UC mở rộng → UC gốc, kèm điều kiện); **MỌI dây include/extend PHẢI có nhãn trên dây: ghi `<<include>>` hoặc `<<extend>>`** (kèm điều kiện cho extend, VD `<<extend>> khi quên MK`); actor bên ngoài khung, UC trong khung hệ thống.
...
2. **MÀU SẮC (user chốt — KHÔNG sáng/basic)**: dùng palette trung tính + teal chủ đạo #0D9488 (đậm vừa, không neon): nền khung hệ thống trắng/`#F8FAFC` (light) · khung viền `#CBD5E1` · actor `#334155` (xám đậm, không đen) · UC elip viền `#0D9488` + chữ `#0F172A` + nền `#F0FDFA` (teal 50) · nhóm module nền `#F1F5F9` viền đứt `#94A3B8` · - Bố cục: mũi tên include `#0D9488` nét liền + **nhãn `<<include>>` trên dây** / extend `#94A3B8` nét đứt + **nhãn `<<extend>> <điều kiện>` trên dây** (font 9-10pt, màu xám `#64748B`, đặt giữa dây, không đè lên đường nét). CẤM màu neon/đỏ tươi/nhiều màu lòe loẹt.3. **BỐ CỤC CHỐNG CHÉO DÂY (user chốt — bắt buộc, thay kiểu "4 UC 1 cột")**:
