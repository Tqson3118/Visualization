...
namespace VisualizationDSA.Domain.Entities
{
    public class ModuleItem
    {
        ...
        public ModuleItem(Guid moduleId, Guid? classroomId, ModuleItemType itemType, Guid? lessonId, Guid? quizId, Guid? codelabId, string overrideTitle, int orderIndex, bool isRequired)
        {
            ...
            // Một "Lesson" có thể mang thêm CodelabId để liên kết Lab ↔ bài giảng lý thuyết (legacy mapping:
            // GetMyEnrollments / MarkRoadmapCompletedIfLastLessonAsync đọc ModuleItem(CodelabId == node.LabId) → LessonId).
            var isLabLessonLink = itemType == ModuleItemType.Lesson && lessonId.HasValue && codelabId.HasValue && !quizId.HasValue;
            if (fkCount != 1 && !isLabLessonLink)
                throw new ArgumentException("ModuleItem must reference exactly one of: Lesson, Quiz, or Codelab (or a Lesson linked to a Codelab).");
