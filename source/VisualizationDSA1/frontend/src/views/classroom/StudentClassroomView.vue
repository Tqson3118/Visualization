<template>
  <div class="student-classroom-view h-full bg-bg-secondary">
...
const hasNextItem = computed(() => !!findNextItem(curriculum.value, currentItemId.value));
...
async function loadClassroom() {
  try {
    ...
    // F5 / reload: resolve currentItem từ query để không mất vị trí đang học
    if (route.query.itemId) {
      currentItemId.value = route.query.itemId as string;
      currentItem.value = findItemById(curriculum.value, currentItemId.value) ?? null;
    }
  } catch (err) {
