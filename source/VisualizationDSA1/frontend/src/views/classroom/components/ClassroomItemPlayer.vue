  ...
  <div class="item-player bg-bg-secondary border border-border-subtle rounded-2xl overflow-hidden">
    ...
    <header class="player-header px-6 py-4 border-b border-border-subtle flex items-center justify-between flex-wrap gap-3">
      ...
      <main class="player-content p-6">
        ...
        <LessonStepTheory
          v-if="item.itemType === 'Lesson' || item.itemType === 'CustomLesson'"
          :title="item.overrideTitle || item.lessonTitle"
          :content="item.contentMd || item.contentMarkdown || item.customLessonContent"
          :sandbox-type="item.sandboxType"
          :sandbox-config="item.sandboxConfig"
          @completeStep="$emit('complete')"
        />
...
const codelabData = ref<any>(null);

watch(
  () => props.item?.itemType + ':' + props.item?.codelabId,
  async (key) => {
    codelabData.value = null;
    if (props.item?.itemType === 'Codelab' && props.item?.codelabId) {
      try {
        codelabData.value = await codelabApi.getCodelab(props.item.codelabId);
      } catch (err) {
        console.error('Failed to load codelab:', err);
      }
    }
  },
  { immediate: true },
