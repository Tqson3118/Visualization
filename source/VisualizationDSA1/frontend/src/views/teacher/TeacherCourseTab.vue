      ...
      <div class="form-row form-row--inline">
        ...
        <div>
          ...
          <select v-model="lessonForm.quizId" class="form-select">
            ...
            <option v-for="q in quizzesForForm" :key="q.id" :value="q.id">{{ q.title }}</option>
          </select>
        </div>
      </div>
      <div class="form-row form-row--inline">
...
const localQuizzes = ref<any[]>([]);
const quizzesForForm = computed(() => props.quizzesList?.length ? props.quizzesList : localQuizzes.value);

async function loadLocalQuizzes(): Promise<void> {
  try {
    const res = await fetch(`${BASE_URL}/api/v1/concepts/quiz/all`, { headers: getAuthHeaders() });
    if (res.ok) localQuizzes.value = await res.json();
...
async function submitCourse() {
  ...
  try {
    ...
    if (res.ok) { 
      ...
      toastStore.success(editingCourseId.value ? 'Cập nhật thành công!' : 'Tạo thành công!'); 
    ...
    else { const err = await res.json(); toastStore.error(err.message || 'Lỗi khi lưu khóa học.'); }
  } catch { toastStore.error('Không thể kết nối máy chủ.'); }
...
async function uploadCoverImage(event: Event) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      toastStore.error(err.message || 'Lỗi tải ảnh.');
    }
  } catch (err) {
    toastStore.error('Không thể kết nối máy chủ để tải ảnh.');
  } finally {
...
async function deleteCourse(courseId: string) {
  if (!(await confirmDialogStore.confirmDialog({ title: 'Xác nhận', message: 'Bạn có chắc chắn muốn xóa khóa học này?', variant: 'danger', confirmText: 'Xác nhận', cancelText: 'Hủy' }))) return;
  try {
    const res = await fetch(`${BASE_URL}/api/v1/concepts/courses/${courseId}`, { method: 'DELETE', headers: getAuthHeaders() });
    if (res.ok) { toastStore.success('Xóa thành công!'); await loadCourses(); }
  } catch (err) { console.error(err); }
}
...
async function submitLesson() {
  ...
  try {
    ...
    if (res.ok) { toastStore.success(editingLessonId.value ? 'Cập nhật thành công!' : 'Tạo thành công!'); const cid = activeCourseForLesson.value.id; cancelLessonEdit(); await loadCourseLessons(cid); await loadCourses(); }
    else { const err = await res.json(); toastStore.error(err.message || 'Lỗi khi lưu bài giảng.'); }
  } catch (err) { console.error(err); }
  finally { submitting.value = false; }
}
...
async function deleteLesson(lessonId: string, courseId: string) {
  if (!(await confirmDialogStore.confirmDialog({ title: 'Xác nhận', message: 'Bạn có chắc chắn muốn xóa bài giảng này?', variant: 'danger', confirmText: 'Xác nhận', cancelText: 'Hủy' }))) return;
  try {
    const res = await fetch(`${BASE_URL}/api/v1/concepts/lessons/${lessonId}`, { method: 'DELETE', headers: getAuthHeaders() });
    if (res.ok) { toastStore.success('Xóa thành công!'); await loadCourseLessons(courseId); await loadCourses(); }
  } catch (err) { console.error(err); }
}
