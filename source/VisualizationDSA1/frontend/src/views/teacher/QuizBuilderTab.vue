<template>
  <section class="quiz-builder-section">
    ...
    <div v-else-if="filteredQuizzes.length === 0" class="empty-state">
      ...
      <h3 class="text-xl font-bold text-white">Chưa có Quiz phù hợp</h3>
      <p class="text-text-muted mt-2 max-w-md">Không tìm thấy Quiz nào khớp với bộ lọc hiện tại</p>
      <button class="btn-primary mt-6" @click="createNewQuiz">
        <BaseIcon name="plus" class="w-4 h-4 inline mr-1" /> Tạo Quiz mới
      </button>
    </div>
              ...
              <td colspan="6" class="accordion-cell">
                ...
                <div v-else class="quiz-detail-panel animate-fade-in">
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" class="btn-add-inline" @click="addNewQuestion(q)">
...
function addNewQuestion(quiz: any) {
...
function debouncedSearch() {
  // Filtering được tính qua computed filteredQuizzes — không cần debounce riêng
}
...
async function saveQuestion(questionData: any) {
  ...
  const quizId = editingQuizForQuestion.value.id;
  if (editingQuestion.value) {
    await updateQuestion(quizId, editingQuestion.value.id, questionData);
  } else {
    await addQuestionToQuiz(quizId, questionData);
  }
  showQuestionForm.value = false;
  editingQuestion.value = null;
  editingQuizForQuestion.value = null;
}
