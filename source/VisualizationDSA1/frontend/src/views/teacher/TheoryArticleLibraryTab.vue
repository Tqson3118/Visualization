...
async function loadArticles(resetPage: boolean = true) {
...
async function togglePublish(article: TheoryArticle) {
  try {
    ...
    if (res.ok) {
      ...
      toastStore.error(err.message || 'Lỗi khi thay đổi trạng thái xuất bản');
    }
  } catch (err) {
...
async function restoreVersion(version: any) {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/api/v1/theory-articles/${article.id}`, {
      method: 'PUT',
      headers: getAuthHeaders(),
      body: JSON.stringify(data)
    });
    if (res.ok) {
      toastStore.success('Đã khôi phục nội dung từ phiên bản này.');
      showVersions.value = false;
      loadArticles();
    } else {
      const err = await res.json();
      toastStore.error(err.message || 'Khôi phục thất bại');
    }
  } catch (err) {
...
async function saveArticle(data: any) {
  try {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      toastStore.error(err.message || 'Lưu thất bại');
    }
  } catch (err) {
