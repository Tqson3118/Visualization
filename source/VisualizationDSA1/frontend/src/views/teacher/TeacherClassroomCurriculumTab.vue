...
const classroomId = computed(() => props.classroomId || (route.query.classroomId as string) || '');
...
async function duplicateItem(item: any, module: any) {
  try {
    await curriculumStore.createItemApi(module.id, {
      moduleId: module.id,
...
async function onDropModule(module: any) {
  ...
  const fromIdx = modules.findIndex(m => m.id === sourceId);
  const toIdx = modules.findIndex(m => m.id === module.id);
  if (fromIdx === -1 || toIdx === -1) return;

  const reordered = [...modules];
  const [moved] = reordered.splice(fromIdx, 1);
  reordered.splice(toIdx, 0, moved);
  const orders = reordered.map((m, idx) => ({ moduleId: m.id, orderIndex: idx }));
  try {
...
function handleDragEnd(event: any) {
  if (event.over && event.active.id !== event.over.id) {
    ...
    const module = curriculum.value?.modules.find(m => m.items.some(i => i.id === fromId));
    if (module) {
      ...
      if (fromIdx !== -1 && toIdx !== -1) {
        ...
        const [moved] = reordered.splice(fromIdx, 1);
        reordered.splice(toIdx, 0, moved);
        const orders = reordered.map((i, idx) => ({ itemId: i.id, orderIndex: idx }));
        void curriculumStore.reorderItemsApi(module.id, currentUserId.value!, orders)
