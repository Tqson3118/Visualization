...
async function handleSubmit() {
  if (props.Type === 'testcase') {
    if (!form.input.trim() || !form.expectedOutput.trim()) {
      toastStore.error('Input và Expected Output không được để trống');
      return;
    }
  } else if (props.Type === 'template') {
    if (!form.language || !form.starterCode.trim()) {
      toastStore.error('Ngôn ngữ và Starter Code không được để trống');
      return;
    }
  } else if (props.Type === 'hint') {
    if (!form.content.trim()) {
      toastStore.error('Nội dung gợi ý không được để trống');
      return;
    }
  }
