...
async function loadClassroomAnalytics() {
  ...
  try {
    const res = await fetch(`${BASE_URL}/api/v1/classrooms/${selectedClassroomId.value}/statistics`, { headers: getAuthHeaders() });
    if (res.ok) analyticsData.value = await res.json();
    else toastStore.error('Không thể tải dữ liệu thống kê của lớp học.');
...
async function exportToExcel() {
  if (!selectedClassroomId.value) return;
  try {
    const res = await fetch(`${BASE_URL}/api/v1/classrooms/${selectedClassroomId.value}/export-excel`, { headers: getAuthHeaders() });
    if (res.ok) {
      ...
      toastStore.error('Không thể xuất báo cáo Excel.');
    }
  } catch (err) {
