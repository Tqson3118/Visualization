<template>
  <section class="quiz-manage-section">
    ...
    <div v-if="activeFormType === 'excel'" class="mb-8 animate-fade-in">
      <div class="p-5 rounded-xl bg-bg-secondary border border-border-subtle">
        <h3 class="form-title-context">
          <BaseIcon name="export-share" class="w-4 h-4 text-accent inline mr-1 align-middle" /> Nhập trắc nghiệm từ CSV
        </h3>
        <p class="text-xs text-text-muted mt-1 mb-3">
          Định dạng mỗi dòng: <code class="font-mono">tiêu đề,chủ đề,độ khó,xp,câu hỏi,đáp án 1,đáp án 2,đáp án 3,đáp án 4,đáp án đúng (0-3),giải thích</code>
        </p>
        <div class="flex flex-wrap gap-2 mb-3">
          <button type="button" class="btn-secondary text-xs px-3 py-1.5" @click="downloadTemplate">
            <BaseIcon name="download" class="w-3.5 h-3.5 inline mr-1" /> Tải template CSV
          </button>
        </div>
        <textarea
          v-model="excelCsv"
          rows="8"
          class="w-full bg-bg-primary border border-border-subtle rounded-xl p-3 text-xs font-mono text-text-primary focus:outline-none focus:border-accent"
          placeholder='Paste nội dung CSV vào đây...'
        ></textarea>
        <div class="flex items-center gap-3 mt-3">
          <button type="button" class="btn-submit" :disabled="importing" @click="handleExcelImport">
            {{ importing ? 'Đang nhập...' : 'Nhập từ CSV' }}
          </button>
          <span v-if="excelCsv.trim()" class="text-xs text-text-muted">Ước tính: {{ estimatedQuizCount }} bài trắc nghiệm</span>
        </div>
      </div>
    </div>
...
async function editQuiz(quizId: string): Promise<void> {
  ...
  } catch (err: any) { toastStore.error(err.message || 'Lỗi khi tải thông tin bài trắc nghiệm'); }
}
...
async function deleteQuiz(quizId: string): Promise<void> {
  const ok = await confirmDialogStore.confirmDialog({
    ...
    message: 'Bạn có chắc chắn muốn xóa bài trắc nghiệm này?',
    variant: 'danger',
  });
  if (!ok) return;
  try {
  ...
  } catch (err: any) { toastStore.error(err.message || 'Lỗi không xác định khi xóa'); }
}
...
function onImportSuccess(): void {
  toastStore.success('Nhập danh sách trắc nghiệm từ CSV thành công!');
  activeFormType.value = 'none'; loadQuizzes(); loadAnalytics();
}

// ── Nhập từ Excel/CSV (P1) ──
const excelCsv = ref('');
const importing = ref(false);

const estimatedQuizCount = computed(() => {
  if (!excelCsv.value.trim()) return 0;
  const titles = new Set<string>();
  for (const line of excelCsv.value.split(/\r?\n/)) {
    const cols = parseCsvLine(line);
    if (cols.length >= 10 && cols[0].trim()) titles.add(cols[0].trim());
  }
  return titles.size;
});
...
function downloadTemplate(): void {
  const template = [
    'Tiêu đề,Chủ đề,Độ khó,XP,Câu hỏi,Đáp án 1,Đáp án 2,Đáp án 3,Đáp án 4,Đáp án đúng,Giải thích',
    '"Array Basics","array","easy",20,"Độ phức tạp truy cập phần tử mảng là?","O(1)","O(N)","O(log N)","O(N^2)",0,"Mảng truy cập trực tiếp theo index."',
  ].join('\n');
...
async function handleExcelImport(): Promise<void> {
  const content = excelCsv.value.trim();
  if (!content) {
    toastStore.error('Vui lòng dán nội dung CSV vào ô phía trên.');
    return;
  }
  ...
  try {
    ...
    for (const line of content.split(/\r?\n/)) {
      ...
      const [title, topic, difficulty, xp, question, o1, o2, o3, o4, ci, explanation] = cols;
      if (!title.trim() || !question.trim()) continue;
      if (!byTitle.has(title.trim())) {
        byTitle.set(title.trim(), {
          title: title.trim(),
          topic: topic.trim() || 'sorting',
          difficulty: difficulty.trim() || 'medium',
          xpReward: parseInt(xp, 10) || 20,
          questions: [],
        });
      }
      byTitle.get(title.trim()).questions.push({
        text: question.trim(),
        options: [o1, o2, o3, o4].map(o => o.trim()).filter(Boolean),
        correctIndex: Math.min(Math.max(parseInt(ci, 10) || 0, 0), 3),
        explanation: (explanation || '').trim(),
      });
    }
    const quizzes = [...byTitle.values()].filter(q => q.questions.length > 0);
    if (quizzes.length === 0) {
      toastStore.error('Không đọc được câu hỏi hợp lệ nào từ CSV. Kiểm tra lại định dạng.');
      return;
    }
    let count = 0;
    ...
    toastStore.success(`Đã nhập ${count}/${quizzes.length} bài trắc nghiệm.`);
    excelCsv.value = '';
    onImportSuccess();
  } catch (err) {
    console.error('CSV import failed:', err);
    toastStore.error('Lỗi khi nhập CSV. Vui lòng kiểm tra lại định dạng.');
  } finally {
