        ...
        <form @submit.prevent="handleSubmit" class="modal-body">
          ...
          <div class="form-field">
            ...
            <div class="linked-content-selector">
              ...
              <p v-if="loadingContent" class="form-hint">
                Đang tải {{ linkedContentLabel.toLowerCase() }}...
              </p>
...
const linkedContentOptionsRaw = ref<LinkedContentOption[]>([]);
...
const itemTypes: Array<{ value: 'Lesson' | 'Quiz' | 'Codelab'; label: string; icon: string }> = [
...
const form = reactive({
  itemType: 'Lesson' as 'Lesson' | 'Quiz' | 'Codelab',
...
const linkedContentOptions = computed<LinkedContentOption[]>(() => linkedContentOptionsRaw.value);

function parseVisualizerAlgo(rawViz: string): string | undefined {
  try {
    const parsed = JSON.parse(rawViz) as { algorithm?: string };
    return parsed.algorithm || undefined;
  } catch {
    return undefined;
  }
}

async function loadLinkedContentOptions(): Promise<void> {
  loadingContent.value = true;
  linkedContentOptionsRaw.value = [];
  try {
    if (form.itemType === 'Lesson') {
      const roadmaps = await teacherStudioService.getMyRoadmaps();
      const options: LinkedContentOption[] = [];
      for (const roadmap of roadmaps) {
        for (const node of roadmap.nodes) {
          options.push({
            id: node.id,
            title: node.name,
            sandboxType: node.visualizerConfig ? parseVisualizerAlgo(node.visualizerConfig) : undefined,
          });
        }
      }
      linkedContentOptionsRaw.value = options;
    } else if (form.itemType === 'Quiz') {
      const quizzes = await quizApi.getAll();
      linkedContentOptionsRaw.value = quizzes.map(q => ({ id: q.id, title: q.title }));
    } else if (form.itemType === 'Codelab') {
      const codelabs = await codelabApi.getCodelabs();
      linkedContentOptionsRaw.value = codelabs.map(c => ({ id: c.id, title: c.title }));
    }
  } catch (err) {
    console.error('Failed to load linked content options:', err);
  } finally {
