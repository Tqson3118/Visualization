  ...
  <div class="lesson-discussion bg-bg-secondary/40 border border-border-subtle rounded-3xl p-6 flex flex-col h-full overflow-hidden text-text-secondary">
    <div class="flex items-center justify-between mb-4 flex-shrink-0">
      ...
      <button 
        ...
        :disabled="loading"
      >
        {{ loading ? 'Đang làm mới...' : 'Làm mới' }} <BaseIcon v-if="!loading" name="refresh" class="w-3.5 h-3.5 inline-block ml-0.5 align-text-bottom" />
      </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto pr-2 scrollbar-thin space-y-4 mb-4">
      ...
      <div v-else v-for="comment in rootComments" :key="comment.id" class="space-y-3">
        ...
        <div class="p-4 bg-bg-surface border border-border-subtle rounded-2xl hover:border-border-subtle transition-all">
          ...
          <div class="mt-3 flex items-center justify-end">
            ...
            >
              <BaseIcon name="arrow-right" class="w-3 h-3 inline-block rotate-180 mr-0.5 align-text-bottom" /> Trả lời
            </button>
          </div>
        </div>
        ...
        <div class="pl-8 space-y-2 border-l border-accent/10">
          <div v-for="reply in getReplies(comment.id)" :key="reply.id" class="p-3 bg-bg-surface border border-border-subtle rounded-xl">
    ...
    <div class="border-t border-border-subtle pt-4 flex-shrink-0 space-y-2">
      ...
      <div class="flex items-center justify-between">
        ...
        >
          {{ submitting ? 'Đang gửi...' : 'Đăng thảo luận' }} <BaseIcon v-if="!submitting" name="message-circle" class="w-3.5 h-3.5 inline-block ml-1 align-text-bottom" />
        </button>
      </div>
    </div>
  </div>
