    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="w-8 h-8 rounded-lg bg-accent flex items-center justify-center">
          <BaseIcon name="trophy" class="w-4 h-4 text-text-primary" />
        </div>
        <div>
      ...
      <div class="flex items-center gap-4">
        ...
        <button
          ...
          :title="'Chưa nối backend — hiển thị Demo'"
        >
          <BaseIcon name="snowflake" class="w-3.5 h-3.5" /> Freeze ({{ store.streakFreezesCount }}) <span class="opacity-60">· Demo</span>
        </button>
        <button v-if="isDevMode" @click="handleAwardXp()" class="px-3 py-1.5 rounded-lg text-xs font-medium bg-accent-green/20 text-accent-green border border-accent-green/30 hover:bg-accent-green/30 transition-colors">+50 XP Demo (DEV)</button>
        <span v-else-if="store.backendProfile" class="text-[10px] font-mono text-text-muted border border-border-subtle rounded px-2 py-1">Demo</span>
      </div>
    </div>
