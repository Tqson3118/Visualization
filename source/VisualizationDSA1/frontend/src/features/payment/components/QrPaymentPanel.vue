  ...
  <div class="grid grid-cols-1 sm:grid-cols-12 gap-6 items-center">
    ...
    <div class="sm:col-span-5 flex flex-col items-center">
      ...
      <div class="mt-3 text-center">
        <div class="text-[10px] text-[var(--color-text-muted)] uppercase font-semibold">Thời gian còn lại</div>
    ...
    <div class="sm:col-span-7 space-y-4">
      ...
      <div class="space-y-2 text-xs bg-bg-secondary/50 p-4 rounded-[var(--radius-lg)] border border-[var(--color-border-default)]">
        <div class="flex justify-between">
          <span class="text-[var(--color-text-muted)]">Ngân hàng:</span>
          <span class="font-bold text-[var(--text-primary)]">{{ order?.bankId }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-[var(--color-text-muted)]">Số tài khoản:</span>
          <span class="font-bold text-[var(--text-primary)] font-mono">{{ order?.bankAccount }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-[var(--color-text-muted)]">Chủ tài khoản:</span>
          <span class="font-bold text-[var(--text-primary)]">{{ order?.accountName }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-[var(--color-text-muted)]">Số tiền:</span>
          <span class="font-bold text-[var(--color-gold)] font-mono">{{ formatCurrency(order?.amount ?? 0) }}</span>
        </div>
        
        <div class="border-t border-[var(--color-border-default)] pt-3 mt-3">
          <div class="text-[10px] text-[var(--color-text-muted)] uppercase mb-1 font-semibold">Nội dung chuyển khoản chính xác:</div>
