...
namespace VisualizationDSA.WebApi.Controllers
{
    ...
    public class LessonController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetLessonById(Guid id)
        {
            ...
            var finalQuizId = linkedQuizId;
            if (!finalQuizId.HasValue && lesson.SandboxType == "quiz" && !string.IsNullOrEmpty(lesson.SandboxConfig))
            {
                try
                {
                    var config = System.Text.Json.JsonDocument.Parse(lesson.SandboxConfig);
                    if (config.RootElement.TryGetProperty("quizId", out var qProp))
                    {
                        var quizIdStr = qProp.GetString();
                        if (Guid.TryParse(quizIdStr, out var parsedGuid))
                        {
                            finalQuizId = parsedGuid;
                        }
                    }
                }
                catch { }
