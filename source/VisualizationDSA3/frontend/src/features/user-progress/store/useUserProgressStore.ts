...
export const useUserProgressStore = defineStore('userProgress', () => {
  ...
  async function loadProgress(): Promise<void> {
    ...
    try {
      ...
      const data = await fetchUserProgress(token, authStore.statelessUser?.id, authStore.isStatelessMode);
      _hydrateFromDto(data);
    } catch (error: unknown) {
      const message = getErrorMessage(error);
      console.warn("⚠️ loadProgress thất bại:", message);
      
      if (error instanceof ApiError && (error.status === 401 || error.status === 404)) {
        console.warn("Phiên đăng nhập không hợp lệ hoặc user không tồn tại. Tự động đăng xuất...");
        if (authStore.isStatelessMode) {
          authStore.statelessLogout();
        } else {
          authStore.logOut();
        }
        return;
  ...
  async function syncXP(amount: number, reason: string): Promise<void> {
    ...
    try {
      ...
      const result = await syncXPToServer(token, payload, authStore.statelessUser?.id, authStore.isStatelessMode);
  ...
  async function _flushPendingQueue(token: string): Promise<void> {
    ...
    for (const payload of queue) {
      ...
      for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
        try {
          await syncXPToServer(token, payload, authStore.statelessUser?.id, authStore.isStatelessMode);
          success = true;
          break;
        } catch (error: unknown) {
          if (error instanceof ApiError && error.status >= 400 && error.status < 500) {
    ...
    try {
      const freshData = await fetchUserProgress(token, authStore.statelessUser?.id, authStore.isStatelessMode);
