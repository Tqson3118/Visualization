<template>
  <header class="app-header z-[999999]" style="position: relative; z-index: 999999 !important;">
    <div class="app-header__inner">
      ...
      <router-link to="/" class="header-logo" style="text-decoration: none; cursor: pointer;">
        <!-- Terminal Dots -->
        <div class="terminal-dots" aria-hidden="true" style="margin-right: 8px;">
        ...
        <!-- Image Logo -->
        <img src="../assets/vite.svg" alt="VisualizationDSA Logo" style="height: 52px; width: auto; object-fit: contain; margin-top: 4px;" />
      </router-link>
      ...
      <div class="header-controls">
        ...
        <template v-if="authStore.isAuthenticated">
          ...
          <div class="relative group z-[999999]">
            <div class="user-badge cursor-pointer" :class="{ 'user-badge--premium': authStore.isPremium }" @click="$router.push('/profile')" title="Xem hồ sơ cá nhân">
            ...
            <!-- My Learning Dropdown on Hover -->
            <div class="absolute right-0 mt-0 w-80 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[999999]" style="top: 100%;">
              <div class="bg-bg-surface border border-border-default rounded-lg shadow-xl p-2 relative z-[999999]">
                  <div v-if="enrolledCourses.length > 0" class="flex flex-col gap-2 p-1">
                    <h3 class="text-xs uppercase tracking-wider text-text-muted font-bold px-2 pb-1 border-b border-border-subtle mb-1">Khóa học của tôi</h3>
                    <div v-for="course in enrolledCourses" :key="course.id" class="flex items-center gap-3 p-2 rounded-lg cursor-pointer hover:bg-bg-hover transition-colors group/course" @click="$router.push(`/courses/${course.id}`)">
                      <img :src="course.coverImageUrl || course.coverImage" class="w-12 h-12 object-cover rounded-md flex-shrink-0 bg-surface-800" />
                      <div class="flex-1 min-w-0">
                        <h4 class="text-sm font-bold text-white group-hover/course:text-accent truncate">{{ course.title }}</h4>
                        <div class="w-full bg-surface-700 h-1.5 rounded-full mt-1.5 overflow-hidden">
                          <!-- Real progress -->
                          <div class="bg-accent h-full transition-all duration-500" :style="{ width: courseStore.getCourseProgress(course.id).progressPercent + '%' }"></div>
                        </div>
                        <span class="text-[10px] text-text-muted mt-0.5 block">{{ courseStore.getCourseProgress(course.id).progressPercent }}% Hoàn thành</span>
                      </div>
                    </div>
                    <router-link to="/courses" class="mt-2 w-full text-center block px-4 py-2 text-sm font-bold text-white bg-accent hover:bg-accent-hover rounded-lg transition-colors">
                      Tiếp tục học
                    </router-link>
                  </div>
                  <div v-else class="text-center py-5 px-3">
                    <p class="text-sm text-text-muted mb-3">Bạn chưa đăng ký khóa học nào.</p>
                    <router-link to="/courses" class="text-accent hover:text-accent-hover text-sm font-bold">
                      Khám phá ngay
                    </router-link>
                  </div>
              </div>
...
const enrolledCourses = computed(() => {
  return courseStore.courses.filter(c => courseStore.isEnrolled(c.id));
});
