<template>
  <div class="lesson-study-view flex h-[calc(100vh-64px)] w-full overflow-hidden bg-bg-secondary font-sans">
    
    <!-- LEFT SIDEBAR: Course Mini Map -->
    <aside class="w-72 lg:w-80 shrink-0 bg-bg-surface border-r border-border-subtle flex flex-col h-full overflow-hidden shadow-xl z-30">
       <!-- Course Title & Search -->
       <div class="p-4 lg:p-5 border-b border-border-subtle shrink-0">
         ...
         <div class="relative">
           ...
           <input v-model="searchQuery" type="text" placeholder="Tìm bài học..." class="w-full bg-bg-secondary border border-border-subtle rounded-xl pl-9 pr-4 py-2 text-xs font-semibold text-white focus:outline-none focus:border-accent transition-colors placeholder:text-text-disabled">
         </div>
       </div>
       ...
       <div class="flex-1 overflow-y-auto p-3 lg:p-4 space-y-1 custom-scrollbar">
         <div v-for="(module, mIdx) in groupedModules" :key="mIdx" class="module-group mb-2">
            <button @click="toggleModule(mIdx)" class="w-full flex items-center justify-between text-left py-2 px-2 rounded-lg group hover:bg-bg-hover transition-colors">
               <span class="text-xs font-bold text-text-secondary group-hover:text-white transition-colors uppercase tracking-wider line-clamp-2 pr-2">{{ mIdx + 1 }}. {{ module.title }}</span>
               <BaseIcon :name="expandedModules.includes(mIdx) ? 'chevron-up' : 'chevron-down'" class="w-4 h-4 text-text-muted transition-transform duration-300 shrink-0" />
            </button>
            
            <div v-if="expandedModules.includes(mIdx)" class="mt-1 ml-[11px] pl-4 border-l-2 border-border-subtle space-y-1 py-1">
               <button v-for="lesson in module.lessons" :key="lesson.id" 
                  ...
                  :class="lesson.id === lessonId ? 'bg-accent/10 text-accent font-bold' : 'text-text-muted hover:text-white hover:bg-bg-hover font-semibold'">
                  
                  <!-- Timeline dot -->
                  <div class="w-[9px] h-[9px] rounded-full absolute -left-[21px] border-2 transition-colors z-10"
                       :class="lesson.id === lessonId ? 'bg-accent border-accent' : 'bg-bg-surface border-border-subtle group-hover:border-text-muted'"></div>
                  
                  <span class="text-xs line-clamp-2 leading-snug">{{ lesson.title }}</span>
               </button>
            </div>
         </div>
         <div v-if="groupedModules.length === 0 && !lessonStore.isLoading" class="text-center text-xs text-text-muted mt-4">
...
const steps = computed(() => {
  if (lessonStore.lessonMeta?.sandboxType === 'quiz') {
    return [FULL_STEPS[2]]; // Chỉ hiện tab Quiz
  }
