<template>
  <div class="lesson-step-quiz relative flex flex-col h-full overflow-hidden font-sans bg-bg-primary">
      ...
      <div class="w-full max-w-3xl flex flex-col gap-3">
        <div class="flex justify-between items-end">
          <h2 class="text-2xl font-black text-white tracking-tight">
            Câu {{ currentIndex + 1 }} <span class="text-text-muted font-medium text-lg">/ {{ questions.length }}</span>
          </h2>
          <span class="text-xs font-semibold" :class="answeredCount === questions.length ? 'text-accent-green' : 'text-accent-yellow'">
        ...
        <div class="w-full h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
          <div 
            ...
            :class="answeredCount === questions.length ? 'bg-gradient-to-r from-accent-green to-emerald-400' : 'bg-gradient-to-r from-accent to-indigo-400'"
        ...
        <div v-if="questions.length > 0" class="mt-4 flex gap-2 flex-wrap justify-center sm:justify-start">
          <button
            ...
            :class="[
              idx === currentIndex ? 'ring-2 ring-accent ring-offset-2 ring-offset-bg-primary text-white bg-accent/80' : '',
              idx !== currentIndex && userAnswers[q.id] !== undefined && !isSubmitted ? 'bg-accent/20 text-accent border border-accent/30 hover:bg-accent/30' : '',
              idx !== currentIndex && userAnswers[q.id] !== undefined && isSubmitted && userAnswers[q.id] === q.correctIndex ? 'bg-accent-green/20 border-accent-green/50 text-accent-green' : '',
              idx !== currentIndex && userAnswers[q.id] !== undefined && isSubmitted && userAnswers[q.id] !== q.correctIndex ? 'bg-accent-red/20 border-accent-red/50 text-accent-red' : '',
              idx !== currentIndex && userAnswers[q.id] === undefined ? 'bg-white/5 border border-white/10 text-text-muted hover:bg-white/10 hover:text-white' : '',
              !isSubmitted && unansweredIndices.includes(idx) && showUnansweredWarning ? 'ring-2 ring-amber-400 ring-offset-1 ring-offset-bg-primary animate-pulse' : '',
            ]"
          >
    ...
    <main class="relative z-10 flex-1 overflow-y-auto px-4 py-4 w-full scroll-smooth">
        ...
        <Transition name="fade-slide" mode="out-in">
          <div :key="currentQuestion.id" class="w-full flex flex-col gap-6">
            ...
            <div class="relative bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
              <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-accent to-purple-500 opacity-50 rounded-t-3xl"></div>
            ...
            <div class="grid grid-cols-1 gap-3">
              <button
                ...
                :class="[
                  userAnswers[currentQuestion.id] === oIdx && !isSubmitted ? 'bg-accent/20 border-accent/50 ring-1 ring-accent shadow-[0_0_20px_rgba(99,102,241,0.2)] text-white' : '',
                  !isSubmitted && userAnswers[currentQuestion.id] !== oIdx ? 'bg-white/5 border-white/10 text-text-secondary hover:bg-white/10 hover:border-white/20 hover:text-white cursor-pointer hover:-translate-y-0.5' : '',
                  isSubmitted && oIdx === currentQuestion.correctIndex ? 'bg-accent-green/20 border-accent-green/50 ring-1 ring-accent-green shadow-[0_0_20px_rgba(16,185,129,0.2)] text-white' : '',
                  isSubmitted && userAnswers[currentQuestion.id] === oIdx && oIdx !== currentQuestion.correctIndex ? 'bg-accent-red/20 border-accent-red/50 ring-1 ring-accent-red text-white' : '',
                  isSubmitted && oIdx !== currentQuestion.correctIndex && userAnswers[currentQuestion.id] !== oIdx ? 'bg-black/20 border-transparent text-text-muted opacity-40 cursor-default' : '',
                ]"
              ...
              >
                ...
                <div 
                  ...
                  :class="[
                    userAnswers[currentQuestion.id] === oIdx && !isSubmitted ? 'bg-accent text-white shadow-lg shadow-accent/40' : '',
                    !isSubmitted && userAnswers[currentQuestion.id] !== oIdx ? 'bg-black/30 text-text-muted group-hover:bg-white/10 group-hover:text-white' : '',
                    isSubmitted && oIdx === currentQuestion.correctIndex ? 'bg-accent-green text-white shadow-lg shadow-accent-green/40' : '',
                    isSubmitted && userAnswers[currentQuestion.id] === oIdx && oIdx !== currentQuestion.correctIndex ? 'bg-accent-red text-white' : '',
                    isSubmitted && oIdx !== currentQuestion.correctIndex && userAnswers[currentQuestion.id] !== oIdx ? 'bg-black/40 text-text-disabled' : '',
                  ]"
                >
            ...
            <Transition name="fade-slide-up">
              <div v-if="isSubmitted" 
                   ...
                   :class="userAnswers[currentQuestion.id] === currentQuestion.correctIndex ? 'border-accent-green/30 bg-accent-green/10' : 'border-accent-red/30 bg-accent-red/10'">
                <div class="absolute top-0 left-0 w-1 h-full" :class="userAnswers[currentQuestion.id] === currentQuestion.correctIndex ? 'bg-accent-green' : 'bg-accent-red'"></div>
                <div class="flex items-center gap-3 mb-3">
                  <div class="p-2 rounded-full" :class="userAnswers[currentQuestion.id] === currentQuestion.correctIndex ? 'bg-accent-green/20 text-accent-green' : 'bg-accent-red/20 text-accent-red'">
                    <BaseIcon :name="userAnswers[currentQuestion.id] === currentQuestion.correctIndex ? 'check' : 'close'" class="w-5 h-5" />
                  </div>
                  <h4 class="text-lg font-bold" :class="userAnswers[currentQuestion.id] === currentQuestion.correctIndex ? 'text-accent-green' : 'text-accent-red'">
      ...
      <div v-else class="flex flex-col items-center justify-center h-full text-center py-20">
        ...
        <p class="text-sm text-text-muted mt-2 max-w-sm">Nội dung trắc nghiệm đang được cập nhật. Vui lòng quay lại sau.</p>
      </div>
    </main>

    <!-- Bottom Action Bar -->
    <div v-if="questions.length > 0" class="relative z-20 border-t border-white/5 bg-bg-secondary/80 backdrop-blur-2xl px-6 py-4 flex items-center justify-between gap-4 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
      
      <!-- Result Summary -->
      <div v-if="isSubmitted" class="flex-1">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full flex items-center justify-center shadow-lg" :class="quizPassed ? 'bg-accent-green text-black' : 'bg-accent-red text-white'">
          ...
          <div>
            ...
            <p class="text-xs font-medium" :class="quizPassed ? 'text-accent-green' : 'text-accent-red'">
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
        <div class="flex items-center gap-2">
          <button
            ...
            :class="currentIndex === 0 ? 'bg-white/5 text-text-disabled cursor-not-allowed' : 'bg-white/10 hover:bg-white/20 text-white cursor-pointer shadow-lg hover:-translate-y-0.5'"
          >
          ...
          <button
            ...
            class="w-11 h-11 rounded-2xl flex items-center justify-center transition-all duration-300"
            :class="currentIndex === questions.length - 1 ? 'bg-white/5 text-text-disabled cursor-not-allowed' : 'bg-accent/20 hover:bg-accent/40 text-accent border border-accent/20 cursor-pointer shadow-lg hover:-translate-y-0.5'"
          >
        ...
        <button
          ...
          :class="answeredCount === questions.length ? 'bg-accent hover:bg-accent-light text-white shadow-[0_0_20px_rgba(99,102,241,0.4)] hover:-translate-y-1 cursor-pointer' : 'bg-white/5 text-text-disabled opacity-60 cursor-not-allowed'"
        >
          <span>Nộp Bài</span>
        ...
        <button
          ...
          @click="completeStep"
          class="px-6 py-2.5 rounded-2xl bg-gradient-to-r from-accent-green to-emerald-500 hover:from-emerald-400 hover:to-emerald-500 text-black text-sm font-bold transition-all duration-300 cursor-pointer shadow-[0_0_20px_rgba(16,185,129,0.4)] flex items-center gap-2 hover:-translate-y-1"
        >
