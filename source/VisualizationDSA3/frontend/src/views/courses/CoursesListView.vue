<template>
  <div class="courses-list-view container mx-auto px-4 py-8 max-w-7xl animate-fade-in">
    ...
    <div v-if="courseStore.error" class="mt-6 text-center py-10 bg-bg-surface rounded-lg border border-border-default">
      <div class="text-5xl mb-4"><BaseIcon name="warning" class="w-14 h-14 text-accent-orange mx-auto" /></div>
      <h3 class="text-xl font-bold text-text-primary">Đang dùng dữ liệu ngoại tuyến (Offline Mode)</h3>
      <p class="text-text-secondary mt-2">{{ courseStore.error }}</p>
      <button
