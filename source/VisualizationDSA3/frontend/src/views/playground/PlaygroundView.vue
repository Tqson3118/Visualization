    ...
    <div class="flex items-center gap-2 px-3 py-2 border-b border-surface/70 bg-surface/30 shrink-0">
      <div class="flex items-center gap-1 bg-surface/60 border border-surface rounded-lg p-1">
        ...
        <button
          ...
          @click="switchMode('flowchart')"
        >
          <BaseIcon name="git-merge" class="w-3.5 h-3.5 inline mr-1 align-middle" />Sơ đồ Code (AST)
        </button>
      </div>
      ...
      <span v-else-if="mode === 'flowchart'" class="text-[10px] text-text-secondary hidden md:block">
        Gõ code bất kỳ, hệ thống sẽ tự động vẽ Sơ đồ luồng (Flowchart) theo thời gian thực.
      </span>
    </div>

    <AlgoPlaygroundWorkspace v-if="mode === 'algo'" class="flex-1 min-h-0" :demo-id="algoDemoId" />
    <LiveFlowchartSandbox v-else-if="mode === 'flowchart'" class="flex-1 min-h-0 w-full" />
...
import LiveFlowchartSandbox from '../../features/flowchart-playground/components/LiveFlowchartSandbox.vue';
import { playgroundAlgoDemos } from '../../features/algo-playground/engine/playgroundAlgoDemos';

type PlaygroundMode = 'free' | 'algo' | 'flowchart';
