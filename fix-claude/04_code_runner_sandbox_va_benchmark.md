...
> **Tại sao có module này?** Code Runner cho phép người học **viết code → chạy ngay trong browser → thấy trace/visual** mà không cần backend thực thi. Benchmark cho phép **so sánh hiệu năng thuật toán** (time/comparisons/swaps) trên nhiều kích thước input. Hai tính năng này biến hệ thống từ "xem demo" thành "thực nghiệm".
>
> **Bài toán giải quyết:**
> - Chạy code người dùng an toàn trong **Web Worker + Babel instrumentation** (không phải container/OS sandbox).
> - Đo `durationMs/comparisons/swaps/writes` có guard `10k steps / 1M loop / 5s timeout` để tránh treo.
> - Lưu kết quả best-effort lên backend (`POST /code-runs`, `POST /benchmarks/run`) để lịch sử/replay, nhưng server **không re-run/attest**.

---
## 1b. Kết luận (audit gốc giữ nguyên)
...
## 2b. Sơ đồ Sequence — Code Runner & Benchmark (bổ sung chuẩn §4.2)

```mermaid
sequenceDiagram
  actor U as Người học
  participant V as CodeRunnerView/BenchmarkPanel
  participant S as Pinia codeRunner Store
  participant W as compileWorker (Web Worker)
  participant E as StepExecutor (Babel AST)
  participant B as Backend API
  U->>V: Nhập code + Run / Chọn benchmark (2-5 keys)
  V->>S: run(code, input) / runMeasureInWorker(key, size)
  S->>W: compileInWorker(code, timeout 5s)
  W->>E: instrument + execute (guard 10k steps / 1M ticks)
  E-->>W: TraceEvent[] / error / timeout
  W-->>S: PlaybackFrame / null
  S->>V: trace/output/stats + playback init
  V->>B: POST /code-runs {trace, stats} (best-effort)
  V->>B: POST /benchmarks/run {Results client}
  B-->>V: CodeRun / Fitted conclusion (server không re-run)
```
...
## 8. Bộ câu hỏi tự kiểm tra (Q&A Self-Test)
