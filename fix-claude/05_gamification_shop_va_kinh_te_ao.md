...
> **Tại sao có module này?** Gamification (EXP/level/gems/quest/streak/hearts) + Shop/Inventory + Premium/VietQR + Leaderboard/Ladder tạo **vòng lặp động lực học tập** (learn → earn → spend → compete). Không có nó, hệ thống chỉ là thư viện thuật toán khô khan, không giữ chân người học.
>
> **Bài toán giải quyết:**
> - **Kinh tế ảo khép kín:** `learning event → XP/level`, `quest claim → gems`, `gems → shop buy → inventory`, `VietQR → premium`.
> - **Cạnh tranh xã hội:** leaderboard theo XP (week/level/class) và ladder learning-path.
> - **Thanh toán nội địa:** VietQR EMVCo TLV + CRC16 offline, server đối soát contentRef `DSV{userId}T{months}`.

---
## 2. Phạm vi và kết luận (audit gốc)
...
### Bảng phân tích File-by-File (bổ sung chuẩn §4.3)

| # | Đường dẫn thật | Hàm / Class trọng tâm | State / Quyết định |
|---|---|---|---|
| 1 | `frontend/src/api/gamification.ts:6-214` | `GamificationSummaryDto`, `buyItem()`, `equipItem()` | client→server ledger; RawQuestDto id number vs Guid drift |
| 2 | `frontend/src/stores/gamification.ts:86-92` | `claimQuest()`, `gems/xp delta` | delta update, không authoritative total |
| 3 | `frontend/src/stores/leaderboard.ts` | `tab/week/level/class`, `effectiveClassId` | UI fix setNoClass; backend chưa filter class |
| 4 | `frontend/src/lib/vietqr.ts` | `tlv()`, `getCrc16()`, EMVCo payload | offline; amount/TLV length chưa validate |
| 5 | `backend/src/DsaVisual.Application/Services/GamificationService.cs:1101-1151` | `AwardXPAsync`, `LevelTable`, `Fitted catalog` | 8 thresholds; không event idempotency |
| 6 | `backend/src/DsaVisual.Api/Controllers/ShopController.cs` | `BuyItem` read-then-write | gems check rồi spend không atomic; thiếu unique inventory |
| 7 | `backend/src/DsaVisual.Api/Controllers/PremiumController.cs` | `Upgrade/MockPay`, `DSV{uid}T{months}` | FE comment mismatch contentRef; mock-pay thiếu idempotency |
| 8 | `backend/src/DsaVisual.Api/Controllers/LeaderboardV2Controller.cs` | `OrderByDescending TotalXP` | tab param nhưng query toàn bộ Student |

> Giữ nguyên inventory chi tiết ở §1–§4 audit gốc.

## 1. Frontend file-by-file (audit gốc)
