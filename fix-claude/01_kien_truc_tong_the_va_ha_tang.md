...
> **Tại sao có module này?** Chặng 1 trả lời câu hỏi nền tảng nhất của đồ án: **hệ thống được phân tầng thế nào và một request đi từ browser tới DB ra sao**. Không hiểu Clean Architecture (Api vs Application), luồng khởi chạy (`Program.cs` ↔ `main.ts` ↔ `router`) và cơ chế Auth (JWT/Refresh/2FA/Role), mọi chặng sau (engine, gamification, admin) sẽ là rời rạc.
>
> **Bài toán giải quyết:**
> - Đối chiếu **Clean Architecture BE** (Controllers mỏng → Application services → EF Core → SQL Server) vs **SPA FE** (Vue 3 Composition + Pinia + Vite + Tailwind).
> - Vẽ luồng **bootstrap**: `main.ts:28 bootstrap()` (Pinia → refresh cookie → fetchMe → router mount) và `Program.cs:29 CreateBuilder` (JWT secret ≥32, CORS, Serilog, ForwardedHeaders).
> - Chứng minh **Auth là security boundary**: access JWT memory-only + refresh HttpOnly cookie + 401 singleton retry + role guard chỉ là UX, backend `[Authorize]` mới quyết định.

---
## 1b. Chương 1 — Kiến trúc tổng thể và hạ tầng (audit gốc giữ nguyên) — Kiến trúc tổng thể và hạ tầng
...
## 8b. Bảng phân tích File-by-File (bổ sung chuẩn §4.3 — không xóa nội dung audit gốc)

| # | Đường dẫn thật | Hàm / Class trọng tâm | State / Quyết định |
|---|---|---|---|
| 1 | `frontend/src/main.ts:28-49` | `bootstrap()`, `createApp`, `useAuthStore.refresh/fetchMe` | Pinia trước Router; token chỉ memory |
| 2 | `frontend/src/router/index.ts:61-69,315-397,401-422` | `createWebHistory`, `beforeEach` guard | requiresAuth / roles / guestOnly — UX guard |
| 3 | `frontend/src/stores/auth.ts:14-114` | `login/register/logout/refresh/fetchMe`, `isAuthenticated/role` | accessToken memory + singleton refresh promise |
| 4 | `frontend/src/api/client.ts:41-150` | Axios `withCredentials`, request/response interceptors | 401 retry 1 lần, 429 Retry-After, ApiError field |
| 5 | `frontend/src/api/auth.ts:4-74` | `login/register/refresh/me` endpoints | refresh POST không body, cookie HttpOnly |
| 6 | `backend/src/DsaVisual.Api/Program.cs:29-157` | `CreateBuilder`, JWT `AddJwtBearer`, CORS, Serilog, ForwardedHeaders | secret ≥32 chars, MapInboundClaims=false |
| 7 | `backend/src/DsaVisual.Api/Controllers/AuthController.cs` | `Login/Register/Refresh/2FA` actions | mỏng, delegate AuthService |
| 8 | `backend/src/DsaVisual.Application/Services/AuthService.cs` | `LoginAsync/RegisterAsync/RotateRefreshToken` | hash SHA256 refresh, OTP 6 số TTL 5m |
| 9 | `backend/src/DsaVisual.Application/Services/TokenService.cs` | `GenerateAccessToken/GenerateRefreshToken` | JWT Bearer, refresh HttpOnly cookie Secure=IsHttps |
| 10 | `backend/src/DsaVisual.Application/Persistence/AppDbContext.cs:6-55` | `DbSet<>`, `ApplyConfigurationsFromAssembly` | Không có Repository layer, DbSet trực tiếp |

> Bảng này bổ sung để thỏa §4.3; nội dung audit gốc ở §2–§7 được giữ nguyên 100%.
