// engines/core/stepExecutor.ts — StepExecutor theo hợp đồng SDD §4.0.3 (Execution-Driven Visualization)
//
// NGUỒN BÊ: source/VisualizationDSA3/frontend/src/core/CompilerStepExecutor.ts (V3 — Babel AST
// instrumentation, giữ block scope/đệ quy/guard vòng lặp). Chọn V3 thay vì V1 vì:
//   - V3 dùng @babel/parser chèn __trackLine/__loopTick theo AST (scope chính xác, đệ quy đúng);
//     V1 cắt dòng bằng regex + hoist biến (phá block scope, lỗi khi đệ quy).
//   - V3 có MAX_STEPS + MAX_LOOP_ITERATIONS + fallbackToRegex + graph/tree/search state.
//
// ĐIỀU CHỈNH (chỉnh tối thiểu, giữ nguyên logic interpreter):
//   1. Thêm MeasureOptions/MeasureCounters/MeasureTimeoutError + hook đo trong compileJavaScript
//      (chế độ KHÔNG trace — phục vụ runMeasure SDD §4.0.3 v2.5).
//   2. Bổ sung ADAPTER EDV: PlaybackFrame → TraceEvent (line/vars/highlight/kind/explanation),
//      runCode (CodeSimulation → RunResult), statsFromTrace, runMeasure.
//   3. Loại bỏ `any` (thay bằng unknown-cast) — quy tắc repo.

import { parse } from '@babel/parser';
import type { Node as BabelNode } from '@babel/types';

// ════════════════════════════════════════════════════════════════════
// 1. HỢP ĐỒNG SDD §4.0.3 (giữ nguyên từ task 1)
// ════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════
// 2. NGUỒN BÊ (V3) — types của interpreter
// ════════════════════════════════════════════════════════════════════
...
/** Bộ đếm thống kê cho chế độ ĐO (runMeasure — SDD §4.0.3 v2.5). */
...
export interface MeasureOptions {
  /** Giới hạn thời gian (ms) — vượt → ném MeasureTimeoutError → runMeasure trả null. */
  timeoutMs: number;
...
export interface CompileOptions {
  ...
  /** Chế độ ĐO không trace (runMeasure): không sinh PlaybackFrame, chỉ đếm thao tác. */
  measure?: MeasureOptions;
}

/** Ném khi chế độ đo vượt timeout — báo hiệu runMeasure trả null (UI hiển thị N/A). */
export class MeasureTimeoutError extends Error {
  constructor() {
    super('Vượt quá thời gian đo (timeout).');
...
// ════════════════════════════════════════════════════════════════════
// 3. INTERPRETER (bê V3, chỉnh tối thiểu cho measure mode — đánh dấu [MEASURE])
// ════════════════════════════════════════════════════════════════════
...
export class CompilerStepExecutor {
  ...
  private static compileJavaScript(sourceCode: string, initialArray: number[], options?: CompileOptions): PlaybackFrame[] {
    ...
    // [MEASURE] deadline cho chế độ đo không trace (SDD §4.0.3: 5 giây/độ đo)
    const deadline = options?.measure ? performance.now() + options.measure.timeoutMs : 0;
    const isMeasure = options?.measure !== undefined;
    ...
    // [MEASURE] chế độ đo: bỏ qua việc tạo frame — chỉ giữ state thật.
    ...
    const trackLine = (lineNum: number, variables: Record<string, unknown>) => {
      ...
      // [MEASURE] kiểm tra timeout theo đồng hồ thật (sync interpreter không thể bị setTimeout cắt).
      if (isMeasure && performance.now() > deadline) {
    ...
    const loopTick = () => {
      ...
      // [MEASURE] vòng lặp thân rỗng cũng phải bị chặn timeout.
      if (isMeasure && performance.now() > deadline) {
        throw new MeasureTimeoutError();
      }
    };
    ...
    const compare = (a: number, b: number) => {
      ...
      // [MEASURE] đếm so sánh.
      if (isMeasure && options?.measure) {
        options.measure.counters.comparisons++;
      }
    ...
    const swap = (a: number, b: number) => {
      ...
      // [MEASURE] đếm hoán đổi.
      if (isMeasure && options?.measure) {
        options.measure.counters.swaps++;
      }
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars -- giữ nguyên chữ ký V3 (tham số trùng tên frames ngoài)
    ...
    const arrayProxy = new Proxy(state.array, {
      ...
      set(target, prop, value) {
        ...
        if (!isNaN(idx)) {
          ...
          if (!inSwap) {
            if (isMeasure && options?.measure) {
              // [MEASURE] đếm ghi mảng ngoài hoán đổi.
              options.measure.counters.writes++;
            } else {
  ...
  private static compilePseudocodeRegex(sourceCode: string, initialArray: number[]): PlaybackFrame[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (!trimmed || trimmed.startsWith('//')) {
        return; // bỏ dòng trống và bình luận
      }
...
// ════════════════════════════════════════════════════════════════════
// 4. ADAPTER EDV (map PlaybackFrame → TraceEvent — SDD §4.0.3)
// ════════════════════════════════════════════════════════════════════

/** Suy TraceKind từ snapshot + description (interpreter cũ không gắn kind sẵn). */
export function inferTraceKind(snap: CanvasStateSnapshot, description: string): TraceKind {
  if (snap.swappingIndices) return 'swap';
  if (snap.comparingIndices) return 'compare';
  if (description.startsWith('Gán')) return 'assign';
  if (description.includes('Bắt đầu vòng lặp') || description.includes('Đang chạy dòng')) return 'loop';
  if (description.includes('Call stack')) return 'call';
  if (description.startsWith('Gọi') || description.includes('gọi')) return 'call';
  if (description.includes('Trả về')) return 'return';
  return 'assign';
}

/** Map snapshot → danh sách id phần tử cần tô (quy ước id: cell:/node:/queue:/stack:). */
export function snapshotHighlights(snap: CanvasStateSnapshot): string[] {
  const ids: string[] = [];
  if (snap.comparingIndices) {
    ids.push(...snap.comparingIndices.map((i) => `cell:${i}`));
  }
  if (snap.swappingIndices) {
    ids.push(...snap.swappingIndices.map((i) => `cell:${i}`));
  }
  if (snap.highlightedIndices) {
    ids.push(...snap.highlightedIndices.map((i) => `cell:${i}`));
  }
  for (const id of snap.activeIds ?? []) ids.push(`node:${id}`);
  for (const id of snap.visitedIds ?? []) ids.push(`node:${id}`);
  for (const id of snap.queueIds ?? []) ids.push(`queue:${id}`);
  for (const id of snap.stackIds ?? []) ids.push(`stack:${id}`);
  return ids;
}

/** Chuyển 1 PlaybackFrame (V3) → TraceEvent (SDD §4.0.3). */
export function toTraceEvent(frame: PlaybackFrame): TraceEvent {
  const snap = frame.canvasStateSnapshot;
  return {
    line: frame.lineNumber,
    vars: { ...(snap.loopVariables ?? {}), array: snap.array },
    highlight: snapshotHighlights(snap),
    kind: inferTraceKind(snap, frame.description),
    explanation: frame.description,
  };
}

/** Thống kê tích lũy từ trace (comparisons/swaps theo kind; writes theo sự kiện "Gán array"). */
export function statsFromTrace(trace: TraceEvent[]): { comparisons: number; swaps: number; writes: number } {
  let comparisons = 0;
  let swaps = 0;
  let writes = 0;
  for (const event of trace) {
    if (event.kind === 'compare') comparisons++;
    else if (event.kind === 'swap') swaps++;
    if (event.explanation.startsWith('Gán')) writes++;
  }
...
/** Trích số dòng từ message lỗi Babel ("... (2:5)") — fallback 0. */
function extractErrorLine(message: string): number {
  const match = message.match(/\((\d+):\d+\)/);
  return match ? Number(match[1]) : 0;
}

/**
 * Chạy code mẫu (CodeSimulation) bằng interpreter V3 → RunResult (SDD §4.0.3).
 * `input` dạng number[] được nạp vào `array` của sandbox.
 */
export function runCode(code: CodeSimulation, input: unknown): RunResult {
  ...
  try {
    ...
    const trace = frames.map(toTraceEvent);
    const stats = statsFromTrace(trace);
    const lastVars = trace.length > 0 ? trace[trace.length - 1].vars : null;
    return {
      trace,
      output: lastVars && 'array' in lastVars ? lastVars.array : array,
      stats: { ...stats, durationMs: Math.round(performance.now() - start) },
    };
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    return {
      trace: [],
      output: null,
      error: { line: extractErrorLine(message), message },
      stats: { comparisons: 0, swaps: 0, writes: 0, durationMs: Math.round(performance.now() - start) },
    };
  }
}
...
export function runMeasure(code: string, input: unknown): { durationMs: number; comparisons: number; swaps: number; writes: number } | null {
  ...
  try {
    CompilerStepExecutor.compileAlgorithm(code, toNumberArray(input), {
      ...
      measure: { timeoutMs: 5000, counters },
    });
    return {
    ...
    // Timeout (MeasureTimeoutError) hoặc code lỗi → không có số đo hợp lệ.
    return null;
  }
}
