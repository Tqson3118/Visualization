...
import SelectionSortVisualizer from '../components/SelectionSortVisualizer.vue';
import InsertionSortVisualizer from '../components/InsertionSortVisualizer.vue';
...
describe('Algorithm Sandbox — Sorting P0/P1 Tests', () => {
  ...
  // ── Visualizer riêng cho Selection/Insertion (dễ nhận diện, khác Bubble) ──
  describe('Selection & Insertion visualizers', () => {
    function frameFor(algo: string, values: number[], overrides: Partial<SortFrame> = {}): SortFrame {
      return {
        stepIndex: 0,
        arrayState: values,
        arrayStateWithIds: values.map((v, id) => ({ id, value: v })),
        comparingIndices: null,
        pivotIndex: null,
        swappedIndices: null,
        sortedIndices: [],
        description: 'test',
        algorithm: algo as SortFrame['algorithm'],
        variables: {},
        ...overrides,
      };
    }

    it('dispatcher: selection → SelectionSortVisualizer', () => {
      const wrapper = mount(SortingVisualizerDispatcher, {
        props: { frame: frameFor('selection', [5, 3, 8]) },
        global: { stubs: { BaseIcon: { template: '<span />' } } },
      });
      expect(wrapper.findComponent({ name: 'SelectionSortVisualizer' }).exists()).toBe(true);
      expect(wrapper.findComponent({ name: 'BubbleSortVisualizer' }).exists()).toBe(false);
    });

    it('dispatcher: insertion → InsertionSortVisualizer', () => {
      const wrapper = mount(SortingVisualizerDispatcher, {
        props: { frame: frameFor('insertion', [5, 3, 8]) },
        global: { stubs: { BaseIcon: { template: '<span />' } } },
      });
    ...
    it('SelectionSortVisualizer: hiển thị badge MIN tại selectionMinIndex', () => {
      const wrapper = mount(SelectionSortVisualizer, {
        props: {
          frame: frameFor('selection', [5, 3, 8], {
      ...
      expect(wrapper.findAll('.vis-min-badge').length).toBe(1);
    });

    it('InsertionSortVisualizer: hiển thị key chip + slot gap khi đang cầm key', () => {
      const wrapper = mount(InsertionSortVisualizer, {
        props: {
          frame: frameFor('insertion', [5, 5, 8], {
            insertionGapIndex: 1,
            variables: { i: 1, j: 0, key: 3 },
          }),
        },
      });
      expect(wrapper.text()).toContain('3');
      expect(wrapper.findAll('.vis-key-chip').length).toBe(1);
      expect(wrapper.findAll('.vis-bar-gap').length).toBe(1);
    });

    it('InsertionSortVisualizer: không hiển thị key chip khi không ở pha cầm key', () => {
      const wrapper = mount(InsertionSortVisualizer, {
        props: { frame: frameFor('insertion', [3, 5, 8], { insertionGapIndex: null, variables: {} }) },
      });
      expect(wrapper.findAll('.vis-key-chip').length).toBe(0);
      expect(wrapper.findAll('.vis-bar-gap').length).toBe(0);
    });
  });
});
