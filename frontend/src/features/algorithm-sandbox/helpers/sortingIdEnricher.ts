...
export function enrichFramesWithIds(frames: SortFrame[]): void {
  ...
  for (let k = 1; k < frames.length; k++) {
    ...
    // LƯU Ý insertion: swappedIndices mô tả "dời" (arr[j+1] = arr[j], key cầm trong tay)
    // — KHÔNG phải hoán vị thật → bỏ qua nhánh này, để greedy ghép theo giá trị
    const swap = currFrame.swappedIndices;
    if (
      swap &&
      currFrame.algorithm !== 'merge' &&
      currFrame.algorithm !== 'insertion' &&
      swap[0] !== swap[1] &&
    ...
    // Insertion Sort: giữ id ỔN ĐỊNH theo vị trí (cả chuỗi frame dùng chung bộ id,
    // không tạo ghost id mới). Shift làm nhân đôi giá trị + key tạm rời mảng nên
    // greedy sẽ tạo id mới liên tục → transition-group nhảy enter/leave → xuất hiện
    // cột thừa rồi biến mất. Giữ id theo vị trí cũ → bộ id bất biến, chỉ đổi giá trị
    // tại chỗ, không bao giờ có cột mới xen vào.
    if (currFrame.algorithm === 'insertion') {
      currFrame.arrayStateWithIds = currArr.map((val, idx) => ({
        id: prevItems[idx]?.id ?? nextId(),
        value: val,
      }));
      continue;
    }
