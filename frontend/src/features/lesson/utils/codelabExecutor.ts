...
export function runCodelabTask(
  ...
  return new Promise<CodelabRunResult>((resolve) => {
    ...
    // Giải nén testCases (Proxy từ Vue) thành plain object để postMessage có thể clone được.
    const rawTestCases = JSON.parse(JSON.stringify(testCases));
    const payload: CodelabWorkerRequest = { requestId, code, testCases: rawTestCases, entryFunction };
