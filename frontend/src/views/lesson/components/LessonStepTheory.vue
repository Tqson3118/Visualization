  ...
  <div class="lesson-step-theory flex flex-col h-full overflow-y-auto p-6 text-vdsa-text font-sans leading-relaxed">
    ...
    <div class="mt-8 pt-6 border-t border-vdsa-border flex items-center justify-between">
      ...
      <button
        ...
        class="px-5 py-2.5 bg-accent hover:bg-vdsa-accent text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-vdsa-accent/30 flex items-center gap-2 cursor-pointer"
      >
...
function renderTable(rows: string[]): string {
  ...
  const thead = header.map(h => `<th class="border border-vdsa-border px-3 py-2 bg-vdsa-surface text-left font-bold text-accent">${h}</th>`).join('');
  const tbody = body.map(row => {
    const cells = splitTableRow(row).map(c =>
      `<td class="border border-vdsa-border px-3 py-2 text-vdsa-secondary">${c}</td>`).join('');
    return `<tr>${cells}</tr>`;
  }).join('');
...
const formattedContent = computed(() => {
  ...
  const transformInline = (text: string): string => {
    ...
    return html
      ...
      .replace(/`([^`]+)`/g, '<code class="bg-vdsa-surface text-accent px-1.5 py-0.5 rounded text-xs font-mono">$1</code>')
      .replace(/\n\n/g, '</p><p class="my-3">');
  };
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i % 3 === 0) {
      ...
      html += '<pre class="rounded-xl bg-vdsa-surface border border-vdsa-border p-3 overflow-x-auto my-3"><code class="text-xs font-mono text-accent leading-relaxed">'
        + code
        + '</code></pre>';
    }
  }
