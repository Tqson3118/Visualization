...
describe('HomeView — redesigned homepage smoke', () => {
  ...
  it('hero hierarchy: h1 + subtitle muted + 2 CTA', () => {
    ...
    expect(wrapper.findAll('.home__cta a').length).toBe(2);
    wrapper.unmount();
  });

  it('stats: heading + BlockToken hero-stat + 2 stat cards có icon Lucide', () => {
    const wrapper = mount(HomeView);
    expect(wrapper.find('.home__stats-head h2').exists()).toBe(true);
  ...
  it('demo: 3 card, mỗi type có thumbnail khác nhau (bars / row / graph)', () => {
    ...
    // icon demo nằm inline cạnh title
