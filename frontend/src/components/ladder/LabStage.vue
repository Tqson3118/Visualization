...
// P1-B2: canvas = vùng dữ liệu LUÔN tối (canvas-ink, block-token data-core/resolved,
// index mono) — quyết định xuyên-nhóm #5; bỏ gradient/emoji/weight 800; easing chuẩn;
// EmptyState component chung khi không có dữ liệu (DESIGN.md §9).
...
<template>
  <section class="lab-stage">
    ...
    <EmptyState
      v-if="array.length === 0"
      icon="database"
      title="Chưa có dữ liệu lab"
      description="Bậc 2 cần một dãy số để thao tác — quay lại sau khi node có mô phỏng gắn kèm."
    />

    <div v-else class="lab-stage__canvas">
      <button
        ...
        :aria-label="`Ô ${idx}: giá trị ${value}`"
...
/* Block dữ liệu — màu trạng thái thuật toán (DESIGN-IDENTITY §1.2) */
.lab-stage__cell {
  width: 64px;
  height: 64px;
  border: 2px solid color-mix(in srgb, var(--color-data-core) 50%, transparent);
  border-radius: var(--radius-md);
  background: color-mix(in srgb, var(--color-data-core) 18%, transparent);
...
.lab-stage__cell--done {
  ...
  background: color-mix(in srgb, var(--color-resolved) 20%, transparent);
  color: var(--color-resolved);
}

.lab-stage__cell--done .lab-stage__cell-idx { color: color-mix(in srgb, var(--color-resolved) 70%, var(--color-index-muted)); }
...
.lab-stage__fail { color: var(--color-warning); font-weight: 500; }
