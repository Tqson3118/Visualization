...
| 22 | 14/08 | Ollama hover-state re-review (2 ảnh CÓ hover): trục phản hồi trực quan 2/5 → **4/5**, thỏa mãn 3/5 → 4/5, trung bình 3.7/5 | Xác nhận decision #20: điểm thấp ban đầu do ảnh tĩnh không thể hiện hover/loading — không phải lỗi thật. Model 3B chấm thẩm mỹ 3/5 (chủ quan, không có vấn đề cụ thể) | Phase 2 |
| 23 | 14/08 | View Transitions API cho route (decision D1 thực thi): guard beforeResolve + `startViewTransition`, fallback Transition CSS khi không hỗ trợ/reduced-motion; App.vue bỏ Transition cũ khi VT active | Verified: `startViewTransition=true` khi navigate, 0 console error, reduced-motion → fallback chuẩn | router + App.vue |
| 24 | 14/08 | axe-core scan toàn route: fix 10+ violations — App.vue `<main>` lồng → div (landmark), `.register__checklist` role="list" thiếu role="listitem", contrast toolbar canvas (chữ tối trên nền canvas-ink → rgba trắng 0.85), heading-order h3→h2 (PseudocodePanel), link-in-text-block (login/register switch underline) | Tiêu chuẩn Phase 2: axe-core 0 critical/serious → đạt 0 violations toàn bộ route public | views + components |
| 25 | 14/08 | Test unit component mới: 14 test (BlockToken/CardPremium/AnimatedNumber/ProgressRing/RevealSection) — tổng suite **109/109 PASS**; test phát hiện bug thật: CardPremium CSS selector `--glow\:resolved` không khớp class render (glow chết im lặng) → đã fix | PROMPT yêu cầu "95 cũ + test mới component"; jsdom thiếu matchMedia/IO → stub + dynamic import | ui-premium.spec.ts |
| 26 | 14/08 | Lenis: đã có toàn cục từ trước (useLenis singleton trong App.vue, autoRaf/anchors/respectReducedMotion) — không cần thêm | DESIGN.md §1 "lenis chọn lọc 1-2 trang" đã thỏa mức tối thiểu | useLenis.ts |
| 27 | 14/08 | Fix bug runtime: RevealSection `ref` trên `<Motion>` trả component instance không phải Element → IntersectionObserver throw; bọc div thật + `@vue:mounted` thay `@vnode-mounted` (Vue 3.5 bỏ vnode-*) | Phát hiện qua console error khi verify VT (playwright); 0 error sau fix | RevealSection.vue + App.vue |
