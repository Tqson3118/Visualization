...
using (var rd = cmd.ExecuteReader())
    while (rd.Read()) rows.Add(rd.GetString(1).PadRight(24) + " -> " + rd.GetString(2).PadRight(24) + " rule=" + rd.GetString(0));
Console.WriteLine("TOTAL FK: " + rows.Count);
Console.WriteLine("--- PART 1 (rows 0-29) ---");
for (int i = 0; i < Math.Min(30, rows.Count); i++) Console.WriteLine(rows[i]);
Console.WriteLine("--- PART 2 (rows 30+) ---");
for (int i = 30; i < rows.Count; i++) Console.WriteLine(rows[i]);
Console.WriteLine("--- CHILD TABLES OF Lessons / LearningPaths / Exercises ---");
foreach (var r in rows) if (r.Contains("-> Lessons") || r.Contains("-> LearningPaths") || r.Contains("-> Exercises") || r.Contains("-> Classes")) Console.WriteLine(r);