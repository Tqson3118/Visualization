...
public interface IClassService
{
    ...
    Task<Result<ClassDetailDto>> JoinAsync(int userId, string role, int id, JoinClassRequest request, CancellationToken ct);

    /// <summary>v2.15: tham gia lớp bằng mã mời (không cần biết classId trước) — POST /classes/join-by-code.</summary>
    Task<Result<ClassDetailDto>> JoinByCodeAsync(int userId, string role, JoinClassByCodeRequest request, CancellationToken ct);
    ...
    Task<Result<List<ClassMemberDto>>> GetMembersAsync(int userId, string role, int id, CancellationToken ct);
    ...
    Task<Result<List<ClassAssignmentDto>>> GetAssignmentsAsync(int userId, string role, int id, CancellationToken ct);
    ...
    Task<Result<ClassDetailDto>> SetLearningPathAsync(int userId, string role, int id, int? learningPathId, CancellationToken ct);
    Task<Result> UpdateLessonDeadlineAsync(int userId, string role, int id, int lessonId, DateTime? dueAt, bool allowLateSubmission, CancellationToken ct);
    Task<Result> UpdateAssignmentDeadlineAsync(int userId, string role, int id, int pathItemId, DateTime? dueAt, bool allowLateSubmission, CancellationToken ct);
